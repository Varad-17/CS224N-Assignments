...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_bias = nn.init.uniform_(nn.Parameter(torch.empty(self.hidden_size)))
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.empty((batch_size,self.embed_size*self.n_features))
        for i in range(batch_size):
            x[i] = torch.flatten(self.embeddings[w[i]])
    ...
    def forward(self, w):
        ...
        inputs = self.embedding_lookup(w)
        output = self.dropout(F.relu(torch.matmul(inputs,self.embed_to_hidden_weight)+ self.embed_to_hidden_bias))
        logits = torch.matmul(output,self.hidden_to_logits_weight) + self.hidden_to_logits_bias
        ### END YOUR CODE
        return logits
