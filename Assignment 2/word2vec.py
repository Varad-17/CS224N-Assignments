...
def naiveSoftmaxLossAndGradient(
    ...
    P = softmax(scores)
    loss = -1*np.log(P[outsideWordIdx])

    temp = P.copy()
    temp[outsideWordIdx] = temp[outsideWordIdx] - 1
    gradCenterVec = np.matmul(outsideVectors.T,temp)

    gradOutsideVecs = np.array([temp[k]* centerWordVec for k in range(len(outsideVectors))])    
...
def negSamplingLossAndGradient(
    ...
    term1 = sigmoid(np.matmul(outsideVectors[outsideWordIdx],centerWordVec))
    term2 = sigmoid(-np.matmul(outsideVectors[negSampleWordIndices],centerWordVec))
    
    loss = - np.log(term1) - np.sum(np.log(term2))
    gradCenterVec = (term1 - 1)*outsideVectors[outsideWordIdx] + np.matmul(outsideVectors[negSampleWordIndices].T,(1 - term2))

    gradOutsideVecs = np.zeros(shape = outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = (term1 - 1)*centerWordVec
    for cnt,index in enumerate(negSampleWordIndices):
        gradOutsideVecs[index] += (1 - term2[cnt])*centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    center_word = centerWordVectors[center_index]
    
    outside_index = [word2Ind[outside_word] for outside_word in outsideWords]

    for outside_idx in outside_index:
        loss_, grad_center, grad_outside = word2vecLossAndGradient(center_word,outside_idx, outsideVectors, dataset)
        loss += loss_
        gradCenterVecs[center_index] += grad_center
